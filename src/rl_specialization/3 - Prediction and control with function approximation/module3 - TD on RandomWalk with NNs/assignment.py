...
def get_value(s, weights):
    ...
    w1 = weights[1]["W"]
    b1 = weights[1]["b"]
    theta = my_matmul(s, w0) + b0
    x = np.maximum(0.0, theta)
    v = my_matmul(x, w1) + b1
    # ----------------
    return v
...
class TDAgent(BaseAgent):
    ...
    def agent_init(self, agent_info={}):
        ...
        for i in range(self.num_hidden_layer+1):
            ...
            self.weights[i]["W"] = self.rand_generator.normal(0, std_dev, (self.layer_size[i],self.layer_size[i+1]))
            self.weights[i]["b"] = self.rand_generator.normal(0, std_dev, (1, self.layer_size[i+1]))
            # ----------------
    ...
    def agent_step(self, reward, state):
        ...
        current_value = get_value(state_vector, self.weights)
